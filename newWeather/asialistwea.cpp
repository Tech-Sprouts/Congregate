#include "asialistwea.h"
//#include "ui_AsiaListWea.h"
...
AsiaListWea::AsiaListWea(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::AsiaListWea)
{
    ui->setupUi(this);
...
AsiaListWea::~AsiaListWea()
{
    delete ui;
}

void AsiaListWea::contextMenuEvent(QContextMenuEvent *event)
{
    //弹出右键菜单
...
void AsiaListWea::mousePressEvent(QMouseEvent *event)
{
    mOffset = event->globalPos() - this->pos();
}

//重写鼠标移动事件
void AsiaListWea::mouseMoveEvent(QMouseEvent *event)
{
    this->move(event->globalPos() - mOffset);
}

//默认情况下先选择五大洲
void AsiaListWea::setState()
{
    ui->listWidget->hide();
...
void AsiaListWea::AddItem(QString city, QString temp,QString pic)
{
    if(city == updateCity){
        //将已获取数据的放入list中
...
void AsiaListWea::on_stateWidget_itemDoubleClicked()
{
...
//回到默认选择大洲状态
void AsiaListWea::on_listBtn_clicked()
{
    ui->listWidget->hide();
    ui->stateWidget->show();
}

void AsiaListWea::getTemp(QString city,QString temp,QString type)
{
    updateCity = city;
    updateTemp = temp;
    updateType = type;
...
//当列表中项目被双击时
void AsiaListWea::on_listWidget_itemDoubleClicked(QListWidgetItem *item)
{
...
void AsiaListWea::on_pushButton_2_clicked()
{
