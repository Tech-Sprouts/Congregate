...
WebView::WebView(QWidget *parent)
    ...
    , m_loadProgress(100)
{
    connect(this, &QWebEngineView::loadStarted, [this]() {//初始为0
        m_loadProgress = 0;
        emit favIconChanged(favIcon());
    });
    connect(this, &QWebEngineView::loadProgress, [this](int progress) {//加载
        m_loadProgress = progress;
    });
    connect(this, &QWebEngineView::loadFinished, [this](bool success) {//成功则为100
        m_loadProgress = success ? 100 : -1;
        emit favIconChanged(favIcon());
    });
    connect(this, &QWebEngineView::iconChanged, [this](const QIcon &) {//图标改变
        emit favIconChanged(favIcon());
    });

    connect(this, &QWebEngineView::renderProcessTerminated,
            [this](QWebEnginePage::RenderProcessTerminationStatus termStatus, int statusCode) {//加载终止
        QString status;
        switch (termStatus) {
 ...
 * @return
 */
QIcon WebView::favIcon() const//网页状态显示
{
    QIcon favIcon = icon();
    if (!favIcon.isNull())
        return favIcon;
 ...
 * @return
 */
QWebEngineView *WebView::createWindow(QWebEnginePage::WebWindowType type)//参数是创建什么类型的窗口
{
    BrowserWindow *mainWindow = qobject_cast<BrowserWindow*>(window());//把qwidget类转为browserwindow类
    if (!mainWindow)
        return nullptr;
